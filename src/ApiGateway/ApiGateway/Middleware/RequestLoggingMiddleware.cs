    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        }))
        {
            _logger.LogInformation(
                "Gateway Request: {Method} {Path}{QueryString} | User: {UserId} | IP: {ClientIp}",
            ...
            finally
            {
                ...
                _logger.Log(
                    logLevel,
                    "Gateway Response: {Method} {Path} | Status: {StatusCode} | Duration: {ElapsedMs}ms | User: {UserId}",
                    context.Request.Method,
                    context.Request.Path,
                ...
                {
                    _logger.LogWarning(
                        "Slow Gateway Request: {Method} {Path} took {ElapsedMs}ms | User: {UserId}",
                        context.Request.Method,
                        context.Request.Path,
                        stopwatch.ElapsedMilliseconds,
                        userId ?? "Anonymous");
